 ...
 * Description:   Shortcode for a call to action in widgets sidebar
 * Version:       1.0
...
Enqueue stylesheet
*********************************************************************************/
function calltoaction_widget_enqueue_styles() {
	
	wp_register_style( 'widget_cta_css', plugins_url( 'css/style.css', __FILE__ ) );
    wp_enqueue_style( 'widget_cta_css' );
 
}
add_action( 'wp_enqueue_scripts', 'calltoaction_widget_enqueue_styles' );
 
/*********************************************************************************
Widget
*********************************************************************************/
class calltoaction_Cta_Widget extends WP_Widget {
	//widget constructor function
	function __construct() {
		$widget_options = array(
			'classname' => 'calltoaction_cta_widget',
			'description' => 'Add a Call to Action box encouraging people to get in touch.'
		);
		parent::__construct( 'calltoaction_cta_widget', 'Call to Action', $widget_options );
	}
...
function calltoaction_register_widget() { 
	register_widget( 'calltoaction_Cta_Widget' );
}
add_action( 'widgets_init', 'calltoaction_register_widget' );